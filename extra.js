...
bot.direction = function(game) {
    ...
    var flpl_ratio = 0;
    var fl_dist = bot.findDistance(game.myBot.pos, game.flowerspi[i].pos);
    var flpl_ratio_i = game.flowerspi[i].pollen / fl_dist;
    if (flpl_ratio < flpl_ratio_i = game.flowerspi[i].pollen / fl_dist) {
        mostFlower = game.flowers[i];
        flpl_ratio = flpl_ratio_i;
    }
    ...
    if (task == "none") {
        ...
        console.log("Heading back to base!")
        myDir = bot.nextStep(game.myBot.pos, game.myBase.pos);
    } 
    else if (task == "steal") {
        console.log("I'm about to do what's called a pro gamer move!")

...
bot.direction = function(game) {
    ...
    else{//find flower distances to each other flower; fd means frequency distribution
        var flowersFDs=[];
            var e;
            for(e=0;e<game.flowers.length;e++){
                for(var i=0;i<game.flowers.length;i++){
                    flowersFDs.push(bot.findDistance(game.flowers[i].pos,game.flowers[e].pos))
                }
            }
        a=0;
    }
    ...
    //expected pollen from path starting from flower 0
    var flowersExpectedPollen = [];
    for(e=0;e<game.flowers.length;e++){
     var expectedPollen = game.flowers[e].pollen;//temporary value of expected pollen from flower 0; will progressively multiply
      for(var d=0;d<=e;d++){//repeat multiply for different flower indices
       var numerator=0;
       for(i=e*(game.flowers.length-1)-d*(game.flowers.length-1);i<(e+1)*(game.flowers.length-1)-d*(game.flowers.length-1);i++){//flowers' probabilities
        if(flowersFDs[i]>bot.findDistance(game.myBot.pos,game.flowers[0].pos)){
            numerator+=1;
        }
       }
    expectedPollen=expectedPollen*(numerator/(game.flowers.length-1));
       }
    flowersExpectedPollen.push(expectedPollen);
    }

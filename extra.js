...
bot.direction = function(game) {
    ...
    var closestFlower = game.flowers[0];
    for (var i = 0; i < game.flowers.length; i++) {
        if (mostFlower.pollen < game.flowers[i].pollen) {
            mostFlower = game.flowers[i];
        }
        game.flowers[i].distance = bot.findDistance(game.myBot.pos, game.flowers[i].pos)
        if (closestFlower.distance > game.flowers[i].distance){
            closestFlower = game.flowers[i];
        }
    }
    ...
    if (task == "none") {
        ...
        if (closestFlower.distance < 10 && closestFlower.pollen > 50){
            myDir = bot.nextStep(game.myBot.pos, closestFlower.pos);
        }
        else {

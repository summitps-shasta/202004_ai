...
bot.direction = function(game) {
    ...
    for (var i = 0; i < enemyBots.length; i++) {
        avg += enemyBots[i].pollen;
    }
    var avgPollen = avg / (enemyBots.length);
    ...
    else if ((turnsLeft < 900) &&
            (game.myBot.pollen < avgPollen)) {
        task = "steal";
        console.log("I'm a Thief!" + 'myPollen:' + game.myBot.pollen + ', ' + 'avgPollen:' +avgPollen);
    }
    else {
        task == "flower";
    }

...
bot.direction = function(game) {
    ...
    for (let i = 0; i < enemyBots.length; i++) {
        if (bot.findDistance(closestEnemy.pos,game.myBot.pos) > bot.findDistance(enemyBots[i].pos,game.myBot.pos)){
    ...
    else if (game.myBot.pollen < enemyMostPollen.pollen && bot.findDistance(game.myBot.pos, mostFlower.pos) > bot.findDistance(game.myBot.pos, enemyMostPollen.pos)) {
        task = "steal";
    } 
    else if(closestEnemy.pollen < game.myBot.pollen && bot.findDistance(closestEnemy.pos,game.myBot.pos) < bot.findDistance(closestEnemy.pos, mostFlower.pos)){
        task = "myBase";
    }
